...
$result = mysqli_query($koneksi, "SELECT p.id_pelanggan, p.nama, p.no_hp, p.email, p.password, t.id_tagihan_lain, t.tagihan, t.status, t.keterangan from tb_pelanggan p inner join tb_tagihan_lain t on p.id_pelanggan=t.id_pelanggan where status='BL' order by tgl_bayar desc");
...
foreach ($result as $key => $data) { //mengirim email untuk setiap baris data
    ...
    $mail->Subject      =  "Yth. " .$data['nama'] . " Ada Tagihan Baru Dari KassandraWiFi Yang Belum Dibayar"; //judul email
    $mail->Body         =  "<p style=font-size:18px>Pelanggan Yth. Sdr/i ".$data['nama'] . 
                           " Terima kasih sudah mempercayai KassandraWiFi sbg layanan hotspot wifi anda </p>".
                           "<table><thead><tr>
                           ...
                           " Lihat Nota Pembayaran</a></p><br><br>Abaikan pesan jika sudah melakukan pembayaran. Terima kasih." .
